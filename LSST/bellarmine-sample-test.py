...
#sites = ['Bellarmine-LSST']
#debug
#sites = ['Bellarmine-LSST','UKI-NORTHGRID-MAN-HEP_LSST', 'UKI-NORTHGRID-LANCS-HEP-LSST']
sites = ['Bellarmine-LSST', 'UKI-NORTHGRID-MAN-HEP_LSST', 'UKI-NORTHGRID-LANCS-HEP-LSST', 'BNL-LSST', 'RAL-LCG2_LSST']
...
for d in directories:
        for r in rafts:
			...
			job.jobDefinitionID   = int(time.time()) % 10000
			job.jobName           = "%s" % commands.getoutput('uuidgen')
			job.transformation    = 'mkdir work output; '
			...
			job.VO                = 'lsst'
			fileOL = FileSpec()
			fileOL.lfn = "dc1-bellarmine-%d.job.log.tgz" % (d)
			fileOL.destinationDBlock = job.destinationDBlock
			fileOL.destinationSE     = job.destinationSE
			fileOL.dataset           = job.destinationDBlock
			...
			#job.jobParameters    = '/cvmfs/lsst.opensciencegrid.org/panda/phosim-phosim_release-0876792bb103/phosim.py    /cvmfs/lsst.opensciencegrid.org/panda/phosim-phosim_release-0876792bb103/examples/star  /cvmfs/lsst.opensciencegrid.org/panda/phosim-phosim_release-0876792bb103/phosim/examples/nobackground  -o $PWD/output -w $PWD/work'
			#job.jobParameters    = 'date; sleep 180; ls /cvmfs'
